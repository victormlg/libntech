...
static int yylex(void *, const char **);
static int yyerror(struct dateinfo *, const char **, const char *);
...
%token  tAGO tID tDST AT_SIGN
%token  <Number>    tDAY tDAYZONE tMINUTE_UNIT tMONTH tMONTH_UNIT
%token  <Number>    tSEC_UNIT tSNUMBER tUNUMBER tZONE tTIME
...
/* ARGSUSED */
static int
yyerror(ARG_UNUSED struct dateinfo *param, ARG_UNUSED const char **inp, ARG_UNUSED const char *s)
{
  (void)yynerrs;
  return 0;
}
...
static int
yylex(void *vval, const char **yyInput)
{
	YYSTYPE *yylval = (YYSTYPE *)vval;
    register char	c;
    register char	*p;
    char		buff[20];
    int			Count;
